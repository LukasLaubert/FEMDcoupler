...
class ParamConfig:
    ...
    output_file = "../dgeba_dedta"  # file name of the output files excluding file extension with (optional) output path
...
#    mesh_coarsening_factor = 3     # coarsens mesh with values >1 by the factor given up to the outermost border; refines mesh with 0<values<1; disable with None or 0 or 1 # NOT IMPLEMENTED
    ...
    # --- MD PARAMETERS ---
    dpd_thickness = None                  # all particles reaching from MD border up to this thickness in each nonperiodic boundary direction are converted to seperate types; set to 0 or None for disabling DPD conversion
    anchor_placement_probability = 1.0  # probability of putting an anchor atom on a particle in the bridge_thickness region; set to 0 for having no anchors
    anchor_symmetric_correction = True  # if true, adjusts the number of anchors to ensure that each opposite region/edge/corner has the same number of anchors
    ...
    notch_width = None        # width of a centered notch along the specified plane (positive and negative values lead to same result; None means "no notch", function is skipped; 0 will result in width of tol)
    notch_normal = 1        # normal direction of the plane where the notch is cut through; the cut is along that plane; 1: x, 2: y; 3: z
    notch_plane = -2        # part's outer surface plane at which the notch starts to be cut through the whole part (cannot be equal to +/-notch_normal); 1: xhi, -1: xlo, 2: yhi, -2: ylo, 3: zhi, -3: zlo
