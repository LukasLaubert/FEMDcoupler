...
class ParamConfig:
    # --- PATH & FILE HANDLING ----
    lammps_file = "../epoxy_anisoNPT_20M.data" # specify a file name with (optional) a path; if "None" (without ""), uses first file with ".data" extension in current path
    output_file = "../zBuilder/theCoupledOutput_sandwich" # file name of the output files excluding file extension with (optional) output path

    # --- FE GEOMETRY PARAMETERS ---
    # (a) rectangular FE systam: set 'circular_burrito_radius = None' below to use
    domain_min_coords = [50, None, None]  # X_min, Y_min, Z_min of FE domain (or None for PBCs in this direction)
    domain_max_coords = [350, None, None]    # X_max, Y_max, Z_max of FE domain (or None for PBCs in this direction)
...
#    mesh_coarsening_factor = 3            # coarsens mesh with values >1 by the factor given up to the outermost border; refines mesh with 0<values<1; disable with None or 0 or 1 # NOT IMPLEMENTED
    ...
    # - NOTCH -
    notch_thickness = 10     # thickness of a centered notch along the specified plane (positive and negative values lead to same result; None means "no notch", function is skipped; 0 will result in thickness of tol)
    notch_normal = 1        # normal direction of the plane where the notch is cut through; the cut is along that plane; 1: x, 2: y; 3: z
    notch_plane = -3        # part's outer surface plane at which the node starts to be cut through the whole part (cannot be equal to +/-notch_normal); 1: pos_x, -1 neg_x, 2: pos_y, -2: neg_y, 3: pos_z, -3: neg_z
